...
class Predict_file_schema_validate:
    """
    Class is used to cross verify schema of passed prediction files with the expected schema for prediction files.
    If any of the files are not matched - Bad Files, then the file will be sent to rejected folder where it will be archived
    """
    def __init__(self, predict_batch_file_path):
        """
        Creates an Instance of Predict_schema_validate class which is used to call defined functions at the time of validation.py
        :param batch_file_path: path of batch training files
        """
        self.predict_batch_file_path = predict_batch_file_path    # predict_batch_file_path
    ...
    def getValuesFromSchema(self):
        try:
            ...
            with open(self.predict_schema_path, 'r') as file:
                dic = json.load(file)
            file.close()
            ...
            log_file = open(self.predict_file_schema_log_filepath, 'a+')
            ...
            message = 'Error in Obtaining values from '+self.predict_schema_path + str(e)
    ...
    def createGoodFileSchemaDataDirectory(self):
        dir_path = 'Validated_Prediction_Batch_Files/GoodFileSchemaDataFolder'
        try:
            log_file = open(self.predict_file_schema_log_filepath, 'a+')
    ...
    def createBadFileSchemaDataDirectory(self):
        dir_path = 'Validated_Prediction_Batch_Files/BadFileSchemaDataFolder'
        try:
            log_file = open(self.predict_file_schema_log_filepath, 'a+')
    ...
    def deleteGooDBadFileSchemaDataFolders(self):
        try:
            ...
            parent_folder = 'Validated_Prediction_Batch_Files/'
    ...
    def validate_file_name(self, regex, LenOfFileDate, LenOfFileTime):
        ...
        # Obtaining the list of files in Prediction_Batch_Files direcctory
        train_batch_files = os.listdir(self.predict_batch_file_path)
        ...
        try:
                ...
                if(re.match(regex, filename)):
                    ...
                    if(len(splitAtUnderScore[2])== LenOfFileDate):
                        ...
                        if(len(splitAtUnderScore[3])== LenOfFileTime):
                            ...
                            shutil.copy(self.predict_batch_file_path+'/'+filename, good_dir_path)

                            # Log the change
                            message = filename +'is validated and accepted, Copying it from '+ self.predict_batch_file_path+'/'+filename+ 'to' + good_dir_path + '\n'
                            self.log_agent.log(log_file, message)

                        else:
                            # copy the file to Bad File Schema_Data path
                            shutil.copy(self.predict_batch_file_path+'/'+filename, bad_dir_path)

                            # Log the change
                            message = filename +' is rejected, Copying it from '+ self.predict_batch_file_path+'/'+filename+ ' to ' + bad_dir_path + '\n'
                            self.log_agent.log(log_file, message)
            ...
            log_file = open(self.predict_file_schema_log_filepath, 'a+')
            message = 'Exception occurred while Validating Prediction_Batch_Files schema ' + str(e)
            self.log_agent.log(log_file, message)
            log_file.close()

    def validate_no_cols(self, accepted_no_cols):
        try:
            good_file_schema_data_dir_path = 'Validated_Prediction_Batch_Files/GoodFileSchemaDataFolder' 
            ...
            for file in filenames:
                ...
                if cols_in_file == accepted_no_cols:
                    ...
                    shutil.move(good_file_schema_data_dir_path+'/'+file, 'Validated_Prediction_Batch_Files/BadFileSchemaDataFolder')
                    
                    #logging the change
                    log_file = open(self.predict_file_schema_log_filepath, 'a+')

                    message = file+" contains "+ str(cols_in_file)+" columns which is less than accepeted " + str(accepted_no_cols) + " columns, so moving file to Validated_Prediction_Batch_Files/BadFileSchemaDataFolder"
            ...
            for file in filenames:
                ...
                for columns in csv:
                    if (len(csv[columns]) != csv[columns].count()):
                        ...
                        # logging the change
                        log_file = open(self.predict_file_schema_log_filepath, 'a+')
                        message = "Invalid length of feature found in file "+ file + "Moving file to  Validated_Prediction_Batch_Files/BadFileSchemaDataFolder"
                        self.log_agent.log(log_file, message)
                        log_file.close()
                        break
                # logging the change
    ...
    def reset_log(self):
        try:
            os.remove(self.predict_file_schema_log_filepath)
            file = open(self.predict_file_schema_log_filepath, 'a+')
            file.close()
        except Exception as e:
            print('Error while resetting '+self.predict_file_schema_log_filepath)
